...
class DjangoStageTransitionStore(StageTransitionStore):
    def commit(self, transition: StageTransition) -> None:
        with transaction.atomic():
            updated = PlanRunModel.objects.filter(identifier=transition.updated_run.identifier, revision=transition.expected_revision).update(
            ...
            if updated != 1:
                raise InvalidStageSubmission("The plan run changed while its stage was being submitted. Retry with its current state.")
