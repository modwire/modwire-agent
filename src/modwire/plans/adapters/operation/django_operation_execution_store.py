...
class DjangoOperationExecutionStore(OperationExecutionStore):
    def completed_operation_ids(self, run_id: UUID) -> set[str]:
        return set(
            OperationExecutionModel.objects.filter(plan_run_id=run_id, status="complete").values_list(
    ...
    def try_claim(self, claim: OperationClaim) -> bool:
        try:
            OperationExecutionModel.objects.create(
                identifier=claim.identifier,
                plan_run_id=claim.plan_run_id,
                operation_id=claim.operation_id,
                execution_key=f"{claim.plan_run_id}:{claim.operation_id}",
                status="pending",
                output={},
            )
    ...
    def is_complete(self, run_id: UUID, operation_id: str) -> bool:
        return OperationExecutionModel.objects.filter(
            execution_key=f"{run_id}:{operation_id}", status="complete"
        ).exists()

    def complete(self, execution: OperationExecution, artifact: PlanArtifact | None) -> None:
        try:
            with transaction.atomic():
                updated = OperationExecutionModel.objects.filter(
                    identifier=execution.identifier, status="pending"
                ).update(status="complete", output=execution.output)
                if updated != 1:
                    raise InvalidStageSubmission("The operation claim is no longer active.")
                if artifact is not None:
                    PlanArtifactModel.objects.create(
                        identifier=artifact.identifier,
                        plan_run_id=artifact.plan_run_id,
                        artifact_id=artifact.artifact_id,
                        artifact_key=f"{artifact.plan_run_id}:{artifact.artifact_id}",
    ...
    def _require_same_result(self, execution: OperationExecution, artifact: PlanArtifact | None) -> None:
        ...
        if artifact is not None:
            existing_artifact = PlanArtifactModel.objects.get(
                artifact_key=f"{artifact.plan_run_id}:{artifact.artifact_id}"
            )
