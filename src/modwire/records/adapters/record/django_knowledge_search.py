...
class DjangoKnowledgeSearch(KnowledgeSearch):
    def semantic(self, query: str) -> list[SearchResult]:
        ...
        rows = ContentSearchProjectionModel.objects.select_related("record").filter(record__status=RecordStatus.PUBLISHED, embedding__isnull=False)
        ...
        scored = [(sum(left * right for left, right in zip(vector, row.embedding)), row) for row in rows]
        return [SearchResult(identifier=row.record_id, title=row.record.title, reason="semantic") for _, row in sorted(scored, key=lambda item: item[0], reverse=True)]

    def text(self, query: str) -> list[SearchResult]:
        if connection.vendor == "postgresql":
            from django.contrib.postgres.search import SearchQuery, SearchRank, SearchVector
            rows = ContentSearchProjectionModel.objects.select_related("record", "revision").filter(record__status=RecordStatus.PUBLISHED).annotate(rank=SearchRank(SearchVector("record__title", "revision__markdown"), SearchQuery(query))).filter(rank__gt=0).order_by("-rank")
        ...
        revisions = ContentRevisionModel.objects.filter(markdown__icontains=query, record__status=RecordStatus.PUBLISHED).select_related("record")
        ...
        return [SearchResult(identifier=record.identifier, title=record.title, reason="text") for record in found.values()]
