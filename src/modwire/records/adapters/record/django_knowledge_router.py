...
class DjangoKnowledgeRouter(KnowledgeRouter):
    def route(self, tag_names: list[str]) -> list[RoutedRecord]:
        records = (
            RecordModel.objects.filter(status=RecordStatus.PUBLISHED, tags__name__in=tag_names)
        ...
        return [
            RoutedRecord(
                ...
                title=record.title,
                matched_tag=next(tag.name for tag in record.tags.all() if tag.name in tag_names),
            )
