from http import HTTPStatus
from typing import Any, cast
from urllib.parse import quote

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.variable_out import VariableOut
from ...models.variable_patch_in_patch import VariablePatchInPatch
from typing import cast


def _get_kwargs(
    variable_id: str,
    *,
    body: VariablePatchInPatch,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "patch",
        "url": "/api/variables/{variable_id}".format(
            variable_id=quote(str(variable_id), safe=""),
        ),
    }

    _kwargs["json"] = body.to_dict()

    headers["Content-Type"] = "application/json"

    _kwargs["headers"] = headers
    return _kwargs


def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> VariableOut | None:
    if response.status_code == 200:
        response_200 = VariableOut.from_dict(response.json())

        return response_200

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[VariableOut]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    variable_id: str,
    *,
    client: AuthenticatedClient,
    body: VariablePatchInPatch,
) -> Response[VariableOut]:
    """Partially update variable.

    Args:
        variable_id (str):
        body (VariablePatchInPatch):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[VariableOut]
    """

    kwargs = _get_kwargs(
        variable_id=variable_id,
        body=body,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    variable_id: str,
    *,
    client: AuthenticatedClient,
    body: VariablePatchInPatch,
) -> VariableOut | None:
    """Partially update variable.

    Args:
        variable_id (str):
        body (VariablePatchInPatch):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        VariableOut
    """

    return sync_detailed(
        variable_id=variable_id,
        client=client,
        body=body,
    ).parsed


async def asyncio_detailed(
    variable_id: str,
    *,
    client: AuthenticatedClient,
    body: VariablePatchInPatch,
) -> Response[VariableOut]:
    """Partially update variable.

    Args:
        variable_id (str):
        body (VariablePatchInPatch):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[VariableOut]
    """

    kwargs = _get_kwargs(
        variable_id=variable_id,
        body=body,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    variable_id: str,
    *,
    client: AuthenticatedClient,
    body: VariablePatchInPatch,
) -> VariableOut | None:
    """Partially update variable.

    Args:
        variable_id (str):
        body (VariablePatchInPatch):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        VariableOut
    """

    return (
        await asyncio_detailed(
            variable_id=variable_id,
            client=client,
            body=body,
        )
    ).parsed
