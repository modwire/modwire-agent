...
class PlanRunScenarios(TestCase):
    def test_runs_a_published_two_stage_definition(self) -> None:
        definition = self.client.post(
            "/api/plans/definitions", data=self._definition(), content_type="application/json"
        )
        run = self.client.post(
            "/api/plans/runs",
            data={"definition_id": definition.json()["id"], "initial_input": {"goal": "replace scaffoldings"}},
            content_type="application/json",
        )
        blocked = self.client.post(
            f"/api/plans/runs/{run.json()['id']}/submissions",
            data={"payload": {"decision": "preserve public seam"}},
            content_type="application/json",
        )
        gate = self.client.post(
            f"/api/plans/runs/{run.json()['id']}/gates/reviewed/satisfactions",
            data={"evidence": {"approved_by": "architect"}},
            content_type="application/json",
        )
        ...
        completed = self.client.post(
            f"/api/plans/runs/{run.json()['id']}/submissions",
            data={"payload": {"outcome": "records is ready"}},
            content_type="application/json",
        )
